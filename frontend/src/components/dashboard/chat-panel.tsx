          ...
          if (event.type === "token" && event.token) {
            ...
            const tokenStr =
              typeof event.token === "string"
                ? event.token
                : (typeof (event.token as { text?: unknown })?.text === "string"
                  ? (event.token as { text: string }).text
                  : "");
              ...
              setMessages((current) =>
                current.map((message) =>
                  message.id === assistantId
                    ? {
                      ...message,
                      content: message.content + tokenStr,
                    }
                    : message,
                ),
              );
      ...
      <aside className={styles.aside}>
        <div className={styles.brandRow}>
          <Link
            ...
            title="Go to Home / Starting Page"
          >
            ...
            <div className={styles.brandText}>
              <p className="font-heading text-lg font-bold tracking-tight bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 bg-clip-text text-transparent">
                MeetAgent AI
              </p>
            </div>
          </Link>
        ...
        <div className={styles.topActions}>
          ...
          <Link
            href="/storage"
            className="flex items-center justify-center gap-1.5 rounded-xl border border-border/70 bg-card/80 px-3 py-2 text-xs font-medium text-foreground transition-all hover:bg-muted"
          >
            ...
            <div className={styles.messagesInner}>
              {showEmpty ? (
                <div className={styles.emptyState}>
                  <div className="mx-auto mb-3 flex size-14 items-center justify-center rounded-2xl bg-card border border-border/80 p-2 shadow-md">
                    <MeetAgentIcon className="size-full" />
                  </div>
                  <h2 className="font-heading text-2xl font-bold tracking-tight bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 bg-clip-text text-transparent">MeetAgent AI</h2>
